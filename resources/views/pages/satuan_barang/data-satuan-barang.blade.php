                ...
                <form action="" class="d-flex col-lg-12">
                  <div class="row col-lg-12">
                    <div class="search col-lg-6">
                      <h6>Kode Satuan Barang</h6>
                      <input type="text" name="satuan_barang_code">
                    </div>
                    <div class="search col-lg-6">
                      <h6>Nama Satuan Barang</h6>
                      <input type="text" name="satuan_barang_name">
                    </div>
                    <div class="submit-filter d-flex  justify-content-between mt-3">
                      ...
                      <a href="{{route('data-satuan-barang')}}" class="btn btn-danger col-lg-1">clear</a>
                    </div>
                  </div>
                </form>
              </div>
          ...
          <div class="card my-3">
            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
              <div class="bg-gradient-success shadow-success d-flex align-items-center justify-content-between border-radius-lg pt-4 pb-3">
                <h6 class="text-white text-capitalize ps-3 mb-0  d-flex align-items-center" style="margin-top:-9px;">Data Satuan Barang</h6>
                <a href="{{route('create-satuan-barang')}}" class="my-0 me-3 btn-add-data d-flex align-items-center">
                ...
                <table class="table align-items-center mb-0" id="myTables">
                  <thead>
                      <tr>
                        ...
                        <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Kode Satuan Barang</th>
                        <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Nama Satuan Barang</th>
                  ...
                  <tbody>
                    @foreach ($satuanBarang as $item)
                      <tr>
                        ...
                        <td class="align-middle text-center text-xs">
                            <p class="text-xs font-weight-bold mb-0 text-center">{{ $item->satuan_barang_code }}</p>
                        </td>
                        <td class="align-middle text-center text-xs">
                            <p class="text-xs font-weight-bold mb-0 text-center">{{ $item->satuan_barang_name }}</p>
                        </td>
                        <td class="align-middle text-center">
                            <span class="text-secondary text-xs font-weight-bolder d-flex justify-content-center align-center">
                              <form action="{{ route('edit-satuan-barang', ['code' => $item->satuan_barang_code]) }}" method="get">
                                @csrf
                              ...
                              <form action="{{ route('delete-satuan-barang', ['code' => $item->satuan_barang_code]) }}" id="delete-form-{{$item->id}}" data-id="{{$item->id}}" method="post">
                                @method('delete')
                                @csrf
