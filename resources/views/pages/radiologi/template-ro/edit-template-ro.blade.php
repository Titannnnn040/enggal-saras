                ...
                <div class="card my-3  border border-0">
                    <div class="card-header p-0 position-relative border border-0 mt-n4 mx-3 z-index-2">
                        <div class="bg-gradient-success shadow-success border-radius-lg pt-4 pb-3">
                            <h6 class="text-white text-capitalize ps-3">Edit Template</h6>
                        </div>
                    </div>
                    <div class="card-body px-5 pb-2">
                        <div class="form"  style="background-color:#FDFEFD;">
                            <div class="content">
                                <form action="{{route('update-template-ro', ['uuid' => $data->uuid])}}" method="post"> 
                                    @csrf     
                                    @method('put')
                                                ...
                                                <div class="col-lg-4 mb-3">
                                                    <div class="d-flex">
                                                        ...
                                                        <div class="d-flex flex-column col-md-9">
                                                            <input type="text" class="form-control @error('template_name') is-invalid @enderror" id="template_name" name="template_name" value="{{ $data->template_name }}">
                                                            @error('template_name')
                                                        ...
                                                        <div class="d-flex flex-column col-md-9">
                                                            <select id="mySelect" class="form-select @error('dokter') is-invalid @enderror" name="dokter"  id="dokter">
                                                                ...
                                                                @foreach ($dokter as $item)
                                                                    <option value="{{$item->no_dokter}}" {{$data->dokter_code == $item->no_dokter ? 'selected' : ''}}>{{$item->nama_lengkap}}</option>
                                                                @endforeach
                                                            </select>          
                                                            @error('dokter')
                                                            <div class="invalid-feedback d-block">
                                            ...
                                            <div class="mb-3">
                                                <div class="d-flex">
                                                    ...
                                                    <div class="d-flex flex-column col-md-10">
                                                        <textarea name="content" id="content" class="form-control @error('content') is-invalid @enderror" id="" cols="30" rows="4">{{ $data->content }}</textarea>
                                                        @error('content')
                                                        <div class="invalid-feedback d-block">
                                                            {{ $message }}
                                                        </div>
